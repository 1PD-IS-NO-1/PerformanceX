...
def analyze_deadline_performance(df):
    """Analyze how well employees meet deadlines with improved data handling"""
    ...
    try:
        ...
        # Group by email and calculate stats, handling empty groups
        deadline_performance = {}
        for email, group in df.groupby("Email"):
            if group["Days to Complete"].notna().any():
                stats = group["Days to Complete"].agg(['mean', 'min', 'max', 'count'])
                deadline_performance[email] = {
                    'mean': stats['mean'],
                    'min': stats['min'],
                    'max': stats['max'],
                    'count': stats['count']
        ...
        print(f"Error in deadline analysis: {str(e)}")
        return {}
...
def calculate_summary_metrics(df):
    try:
        ...
        if 'Number of hours worked' in df.columns:
            df['Number of hours worked'] = (
                df['Number of hours worked']
                .astype(str)
                .str.replace(r'[^\d.]', '', regex=True)  # Fixed regex
                .replace('', '0')
                .astype(float)
            )
        
        total_interns = df['Email'].nunique()
        df['Number of hours worked'] = pd.to_numeric(df['Number of hours worked'], errors='coerce')
        ...
        avg_hours = 0 if pd.isna(avg_hours) else avg_hours
        task_completion = 0 if pd.isna(task_completion) else task_completion
        avg_tasks = 0 if pd.isna(avg_tasks) else avg_tasks
        
        return total_interns, avg_hours, task_completion, avg_tasks
    except Exception as e:
        print(f"Error in calculate_summary_metrics: {str(e)}")
        return 0, 0, 0, 0
...
def calculate_weekly_performance(df):
    ...
    try:
        ...
        # Convert date and calculate week number avg_hours
        df.loc[:, 'Date'] = pd.to_datetime(df["Today's Date"], errors='coerce')
        df = df.dropna(subset=['Date'])
        df.loc[:, 'Week'] = df['Date'].dt.strftime('%Y-W%U')
        
        # Calculate weekly metrics
        weekly_data = df.groupby(['Week', 'Email']).agg({
            'Date': 'count',  # Days present
            'Task Status': lambda x: (x == 'Completed').sum()  # Completed tasks
        }).reset_index()
        
        # Rename columns and calculate additional metrics
        weekly_data = weekly_data.rename(columns={
            'Date': 'DaysPresent',
            'Task Status': 'TasksCompleted'
        })
        
        weekly_data['AttendanceRate'] = (weekly_data['DaysPresent'] / 5 * 100).round(1)
        weekly_data['CompletionRate'] = (weekly_data['TasksCompleted'] / weekly_data['DaysPresent'] * 100).round(1)
        
        return weekly_data.to_dict('records')
    except Exception as e:
        print(f"Error calculating weekly performance: {str(e)}")
        return []

def get_date_range_data(df, start_date, end_date):
    """Filter data by date range"""
    if 'Date' not in df.columns:
        df['Date'] = pd.to_datetime(df["Today's Date"], format="%d/%m/%Y", errors='coerce')
    
    mask = (df['Date'] >= pd.to_datetime(start_date)) & (df['Date'] <= pd.to_datetime(end_date))
    return df.loc[mask]

def calculate_metrics(df):
    """Calculate comprehensive metrics like Streamlit"""
    metrics = {
        'total_interns': df['Email'].nunique(),
        'avg_hours': pd.to_numeric(df['Number of hours worked'], errors='coerce').mean(),
        'completion_rate': (df['Task Status'] == 'Completed').mean() * 100,
        'avg_tasks': df.groupby('Email')['Assigned Task Name'].count().mean(),
        'attendance_rate': df.groupby('Email')['Date'].nunique().mean() / 30 * 100
    }
    return {k: round(v, 2) if isinstance(v, float) else v for k, v in metrics.items()}

def process_dates(df):
    """Handle all date columns consistently"""
    date_cols = ["Today's Date", "Task Assigned Date", "Timestamp"]
...
# Then call this at the start of relevant functions